...
async function telegramWithRetry(method, params) {
  try { return await telegram(method, params); }
  catch (error) {
...
export default async function handler(req, res) {
  ...
  if (chat?.has_protected_content) return json(res, 409, { ok: false, error: 'protected_content_still_enabled' });
  ...
  for (let originalId = FIRST_SOURCE_MESSAGE_ID; originalId <= LAST_SOURCE_MESSAGE_ID; originalId += 1) {
    ...
    try {
      ...
      const normalized = normalizeBotChannelPost({ ...forwarded, message_id: originalId, date: originalDate || forwarded?.date });
  ...
  await patch(TABLES.sources, `id=eq.${encodeURIComponent(source.id)}`, { indexed_at: new Date().toISOString(), indexed_message_count: result.indexed });
