...
export default async function handler(req, res) {
  ...
  const rows = await patch(TABLES.sources, `id=eq.${encodeURIComponent(body.source_id)}`, { indexed_at: new Date().toISOString(), indexed_message_count: Number(body.message_count || 0) });
