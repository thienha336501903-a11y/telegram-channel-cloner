...
export default async function handler(req, res) {
  ...
  try {
    ...
    return json(res, 200, {
      ...
      member: { status: member?.status || null, canPostMessages: Boolean(member?.can_post_messages), canEditMessages: Boolean(member?.can_edit_messages), canDeleteMessages: Boolean(member?.can_delete_messages), canManageChat: Boolean(member?.can_manage_chat) },
      webhook: { configured: Boolean(webhook?.url), url: webhook?.url || null, pendingUpdateCount: Number(webhook?.pending_update_count || 0), lastErrorMessage: webhook?.last_error_message || null }
    });
  } catch (error) {
