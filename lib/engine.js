...
import { finishItem, getMapping, getMappings, getSourceMessage, listDestinations, listSources, logEvent, upsertMapping, updateJobStatus } from './repository.js';

function sleep(ms) { return new Promise((resolve) => setTimeout(resolve, ms)); }

async function loadContext(item) {
  const job = item.job;
  if (!job) throw new Error('Clone job relation missing');
...
async function processCopy(item, ctx) {
  ...
  for (const sourceMessageId of ids) {
    existingRows.push(await getMapping(ctx.source.id, sourceMessageId, ctx.destination.id));
  }
...
async function processRewrite(item, ctx) {
  const mapping = await getMapping(ctx.source.id, ctx.message.source_message_id, ctx.destination.id);
...
export async function runTick(claimFn) {
  ...
  for (const item of items) {
    const destinationId = item.job?.destination_id;
