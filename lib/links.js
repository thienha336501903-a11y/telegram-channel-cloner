...
export function normalizeTelegramSourceRef(value) {
  const raw = String(value || '').trim();
  if (!raw) throw new Error('telegram_source_required');

  if (raw.startsWith('@')) {
    const username = raw.slice(1);
    if (!USERNAME_RE.test(username)) throw new Error('telegram_source_invalid');
    return { chatId: `@${username}`, messageId: null, kind: 'username' };
  }
  if (CHAT_ID_RE.test(raw)) {
    return { chatId: raw, messageId: null, kind: 'chat_id' };
  }
  ...
  try {
    ...
    throw new Error('telegram_source_invalid');
  }
  if (!['http:', 'https:'].includes(url.protocol) || !TELEGRAM_HOSTS.has(url.hostname.toLowerCase())) {
    throw new Error('telegram_source_invalid');
  }

  const parts = url.pathname.split('/').filter(Boolean);
  if (parts[0]?.toLowerCase() === 'c' && /^\d+$/.test(parts[1] || '') && /^\d+$/.test(parts[2] || '')) {
    return { chatId: `-100${parts[1]}`, messageId: Number(parts[2]), kind: 'private_post' };
  }

  const offset = parts[0]?.toLowerCase() === 's' ? 1 : 0;
  const username = parts[offset] || '';
  const messageId = parts[offset + 1] || '';
  if (USERNAME_RE.test(username) && /^\d+$/.test(messageId)) {
    return { chatId: `@${username}`, messageId: Number(messageId), kind: 'public_post' };
  }
  throw new Error('telegram_source_invalid');
}
