...
export async function streamIndexedRange({ index, start, end, streamOriginal, onChunk }) {
  ...
  for (const part of parts) {
    if (part.kind === 'buffer') {
      await onChunk(part.buffer.subarray(part.start, part.end + 1));
    } else {
...
export async function cachedFastStartIndex(cacheKey, builder) {
  const now = Date.now();
  const cached = indexCache.get(cacheKey);
  if (cached && cached.expiresAt > now) return cached.promise;
  const promise = Promise.resolve().then(builder).catch((error) => passthrough(0, error?.message || 'index_failed'));
