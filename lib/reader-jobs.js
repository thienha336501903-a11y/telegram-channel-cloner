...
async function claimFirstQueued(agent, { canReconcile = false } = {}) {
  const typeFilter = canReconcile ? '' : '&job_type=eq.import';
  const candidates = await select(
    TABLES.readerJobs,
    `select=*&status=eq.queued${typeFilter}&order=requested_at.asc&limit=5`
  );
...
export async function claimReaderJob(agentId, capabilities = []) {
  ...
  let claimed = await claimFirstQueued(agent, { canReconcile });
  if (claimed) return claimed;

  if (!canReconcile) return null;
  const due = await queueNextDueReconcileJob();
  if (!due) return null;
  claimed = await claimFirstQueued(agent, { canReconcile: true });
  return claimed;
}
