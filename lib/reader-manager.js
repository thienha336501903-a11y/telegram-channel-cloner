...
export async function listReaderManagerState() {
  const [agents, profiles, access, jobs, sources] = await Promise.all([
    ...
    select(TABLES.readerJobs, 'select=id,source_id,status,job_type,claimed_reader_profile_id,assigned_reader_profile_id,progress_current,progress_total,progress_stage,progress_detail,message_count,deleted_count,error_code,requested_at,claimed_at,heartbeat_at,completed_at,created_at&order=created_at.desc&limit=200'),
    select(TABLES.sources, 'select=id,title,username,chat_id&order=created_at.desc&limit=500')
  ]);
  const now = Date.now();
  ...
  for (const job of jobs || []) {
    const profileId = job.claimed_reader_profile_id || job.assigned_reader_profile_id;
    if (!profileId) continue;
    if (!jobsByProfile.has(profileId)) jobsByProfile.set(profileId, []);
    jobsByProfile.get(profileId).push(job);
  }
  const publicProfiles = (profiles || []).map(profile => {
    const profileJobs = jobsByProfile.get(profile.id) || [];
    const jobsToday = profileJobs.filter(job => Date.parse(job.created_at || 0) >= dayStart.getTime());
    const recentSourceMap = new Map();
    for (const job of profileJobs) {
      if (!job.source_id || recentSourceMap.has(job.source_id)) continue;
      const source = sourceById.get(job.source_id);
      recentSourceMap.set(job.source_id, {
        id: job.source_id,
        title: source?.title || source?.username || 'Kênh Telegram',
        username: source?.username || null,
        last_used_at: job.completed_at || job.claimed_at || job.created_at,
        last_status: job.status
      });
      if (recentSourceMap.size >= 10) break;
    }
    const activeJob = profileJobs.find(job => job.status === 'processing') || null;
    const lastJob = profileJobs.find(job => ['done', 'failed'].includes(job.status)) || profileJobs[0] || null;
    return {
      ...profile,
      effective_status: agentStatus.get(profile.agent_id) === 'online' ? profile.status : 'offline',
      usage_today: {
        jobs: jobsToday.length,
        succeeded: jobsToday.filter(job => job.status === 'done').length,
        failed: jobsToday.filter(job => job.status === 'failed').length,
        messages: jobsToday.reduce((sum, job) => sum + Math.max(0, Number(job.message_count) || 0), 0)
      },
      last_used_at: lastJob?.completed_at || lastJob?.claimed_at || lastJob?.created_at || null,
      active_job: activeJob,
      recent_sources: [...recentSourceMap.values()]
    };
  });
  return {
