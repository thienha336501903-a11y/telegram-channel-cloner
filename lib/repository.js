import { count, insert, patch, select, upsert } from './supabase.js';
...
export async function getActiveSourceByChatId(chatId) {
  const value = String(chatId ?? '').trim();
  if (!value) return null;
  const rows = await select(TABLES.sources, `select=*&active=eq.true&chat_id=eq.${encodeURIComponent(value)}&limit=1`);
  return rows?.[0] || null;
}

// V4 can map many courses to many registered Telegram sources. A source does not
// need to remain the current MASTER to keep receiving/indexing live channel posts.
// The active flag is still used by the clone/mirror subsystem to identify MASTER.
export async function getSourceByChatId(chatId) {
  const value = String(chatId ?? '').trim();
  if (!value) return null;
  const rows = await select(TABLES.sources, `select=*&chat_id=eq.${encodeURIComponent(value)}&limit=1`);
  return rows?.[0] || null;
}
...
export async function syncSourceIndexedMessageCount(sourceId) {
  const value = String(sourceId || '').trim();
  if (!value) return null;

  for (let attempt = 0; attempt < 4; attempt += 1) {
    const rows = await select(TABLES.sources, `select=id,indexed_message_count&id=eq.${encodeURIComponent(value)}&limit=1`);
    const source = rows?.[0];
    if (!source) return null;

    const actual = await count(TABLES.sourceMessages, `source_id=eq.${encodeURIComponent(value)}`);
...
export async function getMapping(sourceId, sourceMessageId, destinationId) {
  const rows = await select(TABLES.messageMappings, `select=*&source_id=eq.${encodeURIComponent(sourceId)}&source_message_id=eq.${sourceMessageId}&destination_id=eq.${encodeURIComponent(destinationId)}&limit=1`);
  return rows?.[0] || null;
}
...
export async function claimQueuedItems(limit) {
  ...
  const items = await select(TABLES.cloneJobItems, `select=*,job:${TABLES.cloneJobs}!inner(id,source_id,destination_id,status)&status=eq.queued&or=(retry_after.is.null,retry_after.lte.${now})&order=created_at.asc&limit=${limit}`);
  const claimed = [];
