...
export async function claimV5MirrorJob(agentId) {
  ...
  const runningJobs = await selectMany(
    `v5_jobs?select=id,payload&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}`
  );

  // Maximum concurrent mirror jobs for any agent is strictly 2 (only for benchmark jobs)
  if (runningJobs && runningJobs.length >= 2) {
    return null;
  }

  // If 1 job is already running, it MUST be a benchmark job to allow claiming a 2nd job
  if (runningJobs && runningJobs.length === 1) {
    const isRunningBenchmark = Boolean(benchmarkPayload(runningJobs[0]));
    if (!isRunningBenchmark) {
      // Production job is running: production concurrency is strictly 1
      return null;
    }
  }
  ...
  // If this agent already has 1 running job, the newly claimed job MUST also be a benchmark job
  if (runningJobs && runningJobs.length === 1) {
    const isClaimedBenchmark = Boolean(benchmarkPayload(claimed));
    if (!isClaimedBenchmark) {
      // Production jobs can never run concurrently with any other job! Requeue it and return null.
...
export async function heartbeatV5MirrorJob({
  ...
  if (stage || bps !== null || eta !== null) {
    const job = await selectOne(
      `v5_jobs?select=id,payload,result&id=eq.${encodeURIComponent(id)}&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}&limit=1`
    );
    if (job && benchmarkPayload(job)) {
