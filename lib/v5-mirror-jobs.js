...
async function finishOwnedJob({
  ...
  if (attempt !== null && attempt !== undefined && job.attempts !== null && job.attempts !== undefined) {
    if (Number(attempt) !== Number(job.attempts)) {
      throw new Error(`v5_mirror_lease_fenced:expected_attempt_${job.attempts}_got_${attempt}`);
    }
  }
  ...
  if (attempt !== null && attempt !== undefined) {
    rpcBody.p_attempt = Number(attempt);
  }

  let rows;
  try {
    rows = await db('rpc/finish_v5_telegram_mirror_job', {
      method: 'POST',
      body: rpcBody
    });
  } catch (err) {
    if (rpcBody.p_attempt !== undefined && String(err?.message || '').includes('Could not find the function')) {
      delete rpcBody.p_attempt;
      rows = await db('rpc/finish_v5_telegram_mirror_job', {
        method: 'POST',
        body: rpcBody
      });
