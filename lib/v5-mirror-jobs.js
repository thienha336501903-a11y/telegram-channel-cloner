...
const V5_MIRROR_LEASE_MS = 15 * 60 * 1000;
...
export function isFreshMirrorLease(lockedAt, nowMs = Date.now()) {
  const lockedMs = Date.parse(String(lockedAt || ''));
  if (!Number.isFinite(lockedMs)) return true;
  return lockedMs >= Number(nowMs) - V5_MIRROR_LEASE_MS;
}
...
export function oneRpcRow(value) {
  if (Array.isArray(value)) {
    return value.length && value[0] && typeof value[0] === 'object' ? value[0] : null;
  }
...
export function validateReportedMirror({
  job,
  asset,
  objectKey,
  bytes,
  ...
  const reportedFinal = safeBytes(finalBytes) ?? safeBytes(bytes);
  if (reportedFinal === null || reportedFinal <= 0) throw new Error('v5_mirror_bytes_required');

  const reportedSource = safeBytes(sourceBytes);
  const assetBytes = safeBytes(asset?.bytes);

  const isPhoto = isPhotoOrThumbnail(asset);
  const version = clean(transformVersion) || 'original-v1';

  if (isPhoto) {
    if (version !== 'original-v1') {
      throw new Error('v5_mirror_faststart_not_supported_for_photos');
    }
    const expectedBytes = assetBytes ?? reportedSource;
    if (expectedBytes !== null && expectedBytes > 0 && reportedFinal !== expectedBytes) {
      throw new Error(`v5_mirror_size_mismatch:${reportedFinal}/${expectedBytes}`);
    }
    return {
      finalBytes: reportedFinal,
      sourceBytes: expectedBytes ?? reportedFinal,
      transformVersion: 'original-v1',
      checksumSha256: clean(checksumSha256) || null
    };
  }

  // Video artifact validation
  // 1. Source size contract: if both reported source and DB asset size exist, they must strictly match
  if (reportedSource !== null && assetBytes !== null && assetBytes > 0 && reportedSource !== assetBytes) {
    throw new Error(`v5_mirror_source_size_mismatch:${reportedSource}/${assetBytes}`);
  }

  const expectedSource = assetBytes ?? reportedSource;
  const reportedBytes = reportedFinal;
  const expectedBytes = expectedSource;

  const cleanedChecksum = clean(checksumSha256) || null;
  ...
  if (version === 'original-v1') {
    ...
    if (cleanedChecksum && !/^[a-f0-9]{64}$/.test(cleanedChecksum)) {
      throw new Error('v5_mirror_checksum_invalid');
    }
  } else if (version === 'ffmpeg-faststart-v1') {
    if (expectedBytes === null || expectedBytes <= 0) {
      throw new Error('v5_mirror_source_bytes_required');
    }
    const maxAllowedDrift = Math.max(1024 * 1024, Math.ceil(expectedBytes * 0.01));
    if (Math.abs(reportedBytes - expectedBytes) > maxAllowedDrift) {
      throw new Error(`v5_mirror_faststart_drift_excessive:${reportedBytes}/${expectedBytes}`);
    }
    if (!cleanedChecksum) {
      throw new Error('v5_mirror_checksum_required');
    }
    if (!/^[a-f0-9]{64}$/.test(cleanedChecksum)) {
      throw new Error('v5_mirror_checksum_invalid');
    }
  } else {
    throw new Error(`v5_mirror_unknown_transform_version:${version}`);
  }

  return {
    finalBytes: reportedFinal,
    sourceBytes: expectedSource ?? reportedFinal,
    transformVersion: version,
    checksumSha256: cleanedChecksum
  };
}
...
async function finishOwnedJob({
  ...
  if (benchmarkPayload(job)) {
    return finishBenchmarkJob({ job, ok, objectKey, bytes: finalBytes || bytes, etag, error, telemetry });
  }
  ...
  let validated = null;
  let reportedBytes = safeBytes(finalBytes) ?? safeBytes(bytes);

  if (ok === true) {
    const asset = await loadMirrorAsset(job.asset_id);
    finalObjectKey = objectKeyFor(job, asset);
    validated = validateReportedMirror({
      job,
      asset,
      objectKey,
      bytes,
    ...
    reportedBytes = validated.finalBytes;
  }
  ...
  const existingResult = finished.result && typeof finished.result === 'object' && !Array.isArray(finished.result)
    ? finished.result
    : {};
  const sanitizedTelemetry = telemetry && typeof telemetry === 'object' && !Array.isArray(telemetry)
  ...
  const patchedResult = {
    ...existingResult,
    ...(validated ? {
      source_bytes: validated.sourceBytes,
      final_bytes: validated.finalBytes,
      transform_version: validated.transformVersion,
      checksum_sha256: validated.checksumSha256,
      faststart_drift_bytes: (validated.sourceBytes !== null && validated.finalBytes !== null)
        ? (validated.finalBytes - validated.sourceBytes)
        : null
    } : {}),
    ...(sanitizedTelemetry ? {
      telemetry: {
  ...
  const patched = await db(`v5_jobs?id=eq.${encodeURIComponent(job.id)}&job_type=eq.telegram_mirror`, {
    ...
    console.error(JSON.stringify({
      level: 'error',
      event: 'v5_mirror_job_result_enrichment_failed',
      job_id: job.id,
  ...
  // Update v5_media_assets with final artifact metadata and checksum
  let assetPatchError = null;
  if (ok === true && validated && job.asset_id) {
    const assetPatch = {
      bytes: validated.finalBytes,
      updated_at: new Date().toISOString()
    };
    if (validated.checksumSha256) {
      assetPatch.checksum_sha256 = validated.checksumSha256;
    }
    await db(`v5_media_assets?id=eq.${encodeURIComponent(job.asset_id)}`, {
      ...
      console.error(JSON.stringify({
        level: 'error',
        event: 'v5_mirror_asset_enrichment_failed',
        job_id: job.id,
  ...
  const finishedJob = one(patched) || { ...finished, result: patchedResult };
...
export async function claimV5MirrorJob(agentId) {
  ...
  const runningRows = await selectMany(
    `v5_jobs?select=id,course_id,asset_id,payload,locked_at&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}`
  );
  // Ignore expired local leases here so the claim RPC can execute its
  // canonical stale-lease requeue before selecting the next candidate.
  // Invalid/missing timestamps fail closed as fresh.
  const runningJobs = runningRows.filter(job => isFreshMirrorLease(job.locked_at));
  ...
  try {
    ...
    await finishOwnedJob({ jobId: claimed.id, agentId: owner, ok: false, error: error?.message || String(error), attempt: claimed.attempts }).catch(() => {});
    throw error;
  }
}
...
export async function heartbeatV5MirrorJob({
  ...
  if (stage || bps !== null || eta !== null) {
    const job = await selectOne(
      `v5_jobs?select=id,course_id,asset_id,payload,result,attempts&id=eq.${encodeURIComponent(id)}&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}&attempts=eq.${encodeURIComponent(parsedAttempt)}&limit=1`
    );
  ...
  const rows = await db(
    `v5_jobs?id=eq.${encodeURIComponent(id)}&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}&attempts=eq.${encodeURIComponent(parsedAttempt)}`,
    {
      method: 'PATCH',
      body: values,
      headers: { Prefer: 'return=representation' }
  ...
  const patched = one(rows);
  if (patched) {
    return patched;
  }

  // Atomic PATCH returned 0 rows: check if fenced or not owned
  const existing = await selectOne(
    `v5_jobs?select=id,status,locked_by,attempts&id=eq.${encodeURIComponent(id)}&limit=1`
  );
  if (!existing) {
    throw new Error('v5_mirror_job_not_owned');
  }
  if (existing.status === 'running' && existing.locked_by === owner && Number(existing.attempts) !== parsedAttempt) {
    throw new Error(`v5_mirror_lease_fenced:expected_attempt_${existing.attempts}_got_${parsedAttempt}`);
  }
  throw new Error('v5_mirror_job_not_owned');
}

export async function finishV5MirrorJob({
  jobId,
  agentId,
  ok,
  objectKey = null,
