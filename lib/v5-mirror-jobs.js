...
function safeMetric(value, max = 86_400_000) {
  const number = Number(value);
  return Number.isFinite(number) && number >= 0 && number <= max ? Math.round(number) : null;
}

function safeMirrorTelemetry(value) {
  if (!value || typeof value !== 'object' || Array.isArray(value)) return null;
  const allowedTimings = new Set([
    'manager_worker_elapsed_ms',
    'manager_first_progress_ms',
    'r2_precheck_ms',
    'telegram_connect_ms',
    'telegram_resolve_ms',
    'telegram_download_ms',
    'faststart_ms',
    'r2_upload_ms',
    'total_ms'
  ]);
  const timings = {};
  const inputTimings = value.timings_ms && typeof value.timings_ms === 'object' ? value.timings_ms : {};
  for (const [key, raw] of Object.entries(inputTimings)) {
    if (!allowedTimings.has(key)) continue;
    const metric = safeMetric(raw);
    if (metric !== null) timings[key] = metric;
  }

  const bytes = {};
  const inputBytes = value.bytes && typeof value.bytes === 'object' ? value.bytes : {};
  for (const key of ['telegram', 'uploaded']) {
    const metric = safeBytes(inputBytes[key]);
    if (metric !== null) bytes[key] = metric;
  }

  const cache = {};
  const inputCache = value.cache && typeof value.cache === 'object' ? value.cache : {};
  for (const key of ['telegram_reused', 'faststart_reused', 'r2_already_complete', 'r2_small_put']) {
    if (typeof inputCache[key] === 'boolean') cache[key] = inputCache[key];
  }

  return {
    version: 1,
    ...(Object.keys(timings).length ? { timings_ms: timings } : {}),
    ...(Object.keys(bytes).length ? { bytes } : {}),
...
async function loadOwnedMirrorContext(jobId, agentId) {
  ...
  const job = await selectOne(`v5_jobs?select=id,course_id,asset_id,status,locked_by,attempts,max_attempts,payload&id=eq.${encodeURIComponent(id)}&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}&limit=1`);
  if (!job) throw new Error('v5_mirror_job_not_owned');
  const asset = await selectOne(`v5_media_assets?select=id,origin,telegram_source_id,telegram_message_row_id,original_filename,mime_type,bytes,status,metadata&id=eq.${encodeURIComponent(job.asset_id)}&limit=1`);
  if (!asset) throw new Error('v5_mirror_asset_missing');
  if (asset.origin !== 'telegram') throw new Error('v5_mirror_asset_not_telegram');
  return { job, asset };
}

async function persistMirrorTelemetry(jobId, agentId, telemetry) {
  const safe = safeMirrorTelemetry(telemetry);
  if (!safe) return null;
  const id = clean(jobId);
  const owner = clean(agentId);
  if (!id || !owner) return null;
  const job = await selectOne(`v5_jobs?select=id,payload&id=eq.${encodeURIComponent(id)}&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}&limit=1`);
  if (!job) return null;
  ...
  const rows = await db(`v5_jobs?id=eq.${encodeURIComponent(id)}&job_type=eq.telegram_mirror&status=eq.running&locked_by=eq.${encodeURIComponent(owner)}`, {
    method: 'PATCH',
    body: { payload: { ...payload, reader_telemetry: safe } },
...
async function finishOwnedJob({ jobId, agentId, ok, objectKey = null, bytes = null, etag = null, error = null, telemetry = null }) {
  ...
  if (ok === true) {
    ...
    if (!isPhotoOrThumbnail && expectedBytes !== null && expectedBytes > 0 && reportedBytes !== expectedBytes) {
      throw new Error(`v5_mirror_size_mismatch:${reportedBytes}/${expectedBytes}`);
    }
  }

  // Telemetry is diagnostic only. A failure to persist it must never fail or
  // delay the canonical mirror completion path.
  await persistMirrorTelemetry(jobId, agentId, telemetry).catch(() => null);
...
export async function finishV5MirrorJob({ jobId, agentId, ok, objectKey, bytes, etag, error, telemetry }) {
