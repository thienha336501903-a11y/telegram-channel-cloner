...
export function validateReportedMirror({
  ...
  const expectedSource = safeBytes(sourceBytes) ?? safeBytes(asset.bytes);
  const reportedBytes = reportedFinal;
  const expectedBytes = expectedSource;

  if (isPhotoOrThumbnail(asset)) {
    if (expectedBytes !== null && expectedBytes > 0 && reportedBytes !== expectedBytes) {
      throw new Error(`v5_mirror_size_mismatch:${reportedBytes}/${expectedBytes}`);
    }
    return {
      finalBytes: reportedBytes,
      sourceBytes: expectedBytes,
      transformVersion: clean(transformVersion) || 'original-v1',
  ...
  if (version === 'original-v1') {
    ...
    if (expectedBytes !== null && expectedBytes > 0) {
      const maxAllowedDrift = Math.max(1024 * 1024, Math.ceil(expectedSource * 0.01));
