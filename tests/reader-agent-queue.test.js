...
assert.match(jobs, /chooseReaderProfileForSource/);
assert.match(jobs, /Claim only jobs that were explicitly queued/);
assert.doesNotMatch(jobs, /READER_RECONCILE_INTERVAL_HOURS/);
assert.doesNotMatch(jobs, /DEFAULT_RECONCILE_HOURS/);
assert.doesNotMatch(jobs, /reconcileIntervalMs/);
assert.doesNotMatch(jobs, /queueNextDueReconcileJob/);
assert.doesNotMatch(jobs, /last_reconciled_at\.lt/);
