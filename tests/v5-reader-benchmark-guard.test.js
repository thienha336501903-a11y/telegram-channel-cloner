...
test('benchmark object key is isolated while exact Phase 4 canary retains canonical production key', () => {
...
test('benchmark heartbeat telemetry is forwarded from API and remains persisted into result.telemetry', () => {
...
test('normal production heartbeat remains unchanged; telemetry exception is only exact server-authorized Phase 4 canary', () => {
  ...
  assert.match(jobs, /const persistTelemetry = job && \(benchmarkPayload\(job\) \|\| await isProductionCanaryJob\(job\)\);/);
  assert.match(jobs, /async function isProductionCanaryJob\(job\)/);
  assert.match(jobs, /clean\(job\?\.course_id\) !== PHASE4_CANARY_COURSE_ID/);
  ...
  // Default values patch only lock/update/progress. result is added only inside persistTelemetry.
...
test('Reader client ordinary production remains concurrency 1 and all local mirror concurrency is capped at 2', () => {
