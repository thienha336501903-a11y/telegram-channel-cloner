...
test('Reader 1.4.9 exposes explicit one-shot V5 mode without changing the default', () => {
  assert.match(agent, /APP_VERSION = "1\.4\.9"/);
  assert.match(installer, /#define MyAppVersion "1\.4\.9"/);
...
test('Reader 1.4.9 installer never auto-launches and GUI visibly identifies version', () => {
  assert.match(agent, /APP_VERSION = "1\.4\.9"/);
  assert.match(installer, /#define MyAppVersion "1\.4\.9"/);
